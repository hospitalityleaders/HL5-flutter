...
class _HeaderState extends State<Header> {
  ...
  List items = ['Home', 'Profile', 'News', 'Jobs', 'Recruitment', 'Help'];
  Widget buildHeaderGesture(
      String menuName, TextStyle _fontStyle, BuildContext context, int index) {
    return GestureDetector(
      onTap: () {
        _controller.setString(widget.page!.capitalizeFirst);
        // Routemaster.of(context).push('/${items[index]}');
        // Navigator.pushNamed(context, '/${items[index]}');
        _controller.setIndex(index);

        // print('---menuIndex----${_controller.menuIndex}');
        // print('----index---${items[index]}');
        // print(
        //     '---capitalizeFirst----${items.indexOf(widget.page?.capitalizeFirst)}');
      },
  ...
  Widget build(BuildContext context) {
    ...
    // print('---  _controller.menuIndex----${_controller.menuIndex}');
    return Container(
        height: 46,
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: List.generate(
                    items.length,
                    (index) => buildHeaderGesture(
                        '${items[index]}',
                        _controller.menuIndex == index
